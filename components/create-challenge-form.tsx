...
import type { ChallengeMode, ChecklistItem } from "@/lib/demo-data";

const fallbackChecklist: ChecklistItem[] = [
  { id: "public", criterion: "The immutable snapshot is publicly accessible and identifies the submitted work.", weight: 15, kind: "REQUIRED" },
  { id: "outcome", criterion: "The evidence directly demonstrates the outcome described in the challenge.", weight: 45, kind: "REQUIRED" },
  { id: "reproducible", criterion: "A reviewer can reproduce or independently verify the claimed result.", weight: 25, kind: "REQUIRED" },
  { id: "quality", criterion: "The result is complete, clear, and suitable for real use.", weight: 15, kind: "QUALITY" },
];
...
export function CreateChallengeForm() {
  ...
  async function draft(event: FormEvent) {
    ...
    try {
      ...
      const hash = await send("Generate challenge checklist", async (client) => client.writeContract({
        ...
        args: [nextId, form.title, form.rules, Math.floor(new Date(form.deadline).getTime() / 1000), form.mode, Number(form.minScore)],
        value: 0n,
      ...
      }) as unknown as { checklist?: ChecklistItem[] };
      if (Array.isArray(stored?.checklist)) setChecklist(stored.checklist);
      setStep(2);
  ...
  async function confirm() {
    setBusy(true);
    try {
      await send("Confirm rubric and open funding", async (client) => client.writeContract({
    ...
    <div className="create-layout">
      ...
      {step === 1 ? (
        <form className="panel form-panel" onSubmit={draft}>
          ...
          <p className="form-note">This sends a real nondeterministic Bradbury transaction. If consensus flags vague rules, funding remains locked.</p>
        </form>
      ) : (
        <section className="panel rubric-review">
          <span className="eyebrow">Step 02 · Creator confirmation</span>
          <div className="review-heading"><div><h1>This is exactly how your challenge will be judged.</h1><p className="lead">Read every criterion. Confirmation makes the rubric immutable.</p></div><span className="consensus-seal">CONSENSUS<br />PROPOSED</span></div>
          ...
          <div className="confirm-bar">
            <label>Initial pool (GEN)<input value={form.pool} onChange={(e) => setForm({ ...form, pool: e.target.value })} /></label>
            <button className="button button-ghost" onClick={() => setStep(1)}>Revise rules</button>
            <button className="button button-primary" onClick={() => void confirm()} disabled={busy}>{busy ? "Opening…" : "Confirm & fund"}</button>
          </div>
